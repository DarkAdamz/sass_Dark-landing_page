...
const AccordionItem = ({question, answer}: {question: string; answer: string}) => {
    ...
    return (
        <div key={question} className="py-7 border-b border-white/30 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
            <div className="flex w-full items-centerflex-1 text-lg font-bold">
                <span>{isOpen ? <MinusIcon /> : <PlusIcon />}</span> {question}
            </div>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
        ...
        <>
            <div className="bg-black text-white bg-gradient-to-b from-[#5d2cab] to-black py-[72px] sm:py-24">
                <div className="container">
                    <h2 className="text-center text-5xl sm:text-6xl sm:max-w-[660px] font-bold tracking-tighter mx-auto">
